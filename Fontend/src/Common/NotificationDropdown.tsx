...
const notification: Notification[] = [
    { id: 1, type: "follower", imageClassName: "w-10 h-10 rounded-md shrink-0 bg-slate-100", image: avatar3, boldName: "@willie_passem", name: "followed you", time: "4 sec", date: "Wednesday 03:42 PM" },
    { id: 2, type: "mention", imageClassName: "w-10 h-10 bg-yellow-100 rounded-md shrink-0", image: avatar5, boldName: "@caroline_jessica", name: "commented on your post", time: "15 min", description: "Amazing! Fast, to the point, professional and really amazing to work with them!!!", date: "Wednesday 03:42 PM", },
    { id: 3, type: "invite", imageClassName: "w-10 h-10 rounded-md shrink-0 bg-slate-100", name: "Successfully purchased a business plan for", price: "$199.99", time: "Yesterday", date: "Monday 11:26 AM" },
    ...
    return (
        <>
            <Dropdown className="relative flex items-center h-header">
                <Dropdown.Trigger type="button" className="inline-flex justify-center relative items-center p-0 text-topbar-item transition-all w-[37.5px] h-[37.5px] duration-200 ease-linear bg-topbar rounded-md dropdown-toggle btn hover:bg-topbar-item-bg-hover hover:text-topbar-item-hover group-data-[topbar=dark]:bg-topbar-dark group-data-[topbar=dark]:hover:bg-topbar-item-bg-hover-dark group-data-[topbar=dark]:hover:text-topbar-item-hover-dark group-data-[topbar=brand]:bg-topbar-brand group-data-[topbar=brand]:hover:bg-topbar-item-bg-hover-brand group-data-[topbar=brand]:hover:text-topbar-item-hover-brand group-data-[topbar=dark]:dark:bg-zink-700 group-data-[topbar=dark]:dark:hover:bg-zink-600 group-data-[topbar=brand]:text-topbar-item-brand group-data-[topbar=dark]:dark:hover:text-zink-50 group-data-[topbar=dark]:dark:text-zink-200 group-data-[topbar=dark]:text-topbar-item-dark" id="notificationDropdown" data-bs-toggle="dropdown">
                    <BellRing className="inline-block w-5 h-5 stroke-1 fill-slate-100 group-data-[topbar=dark]:fill-topbar-item-bg-hover-dark group-data-[topbar=brand]:fill-topbar-item-bg-hover-brand"></BellRing>
                                        ...
                                        {index === notification.length - 1 ?
                                            ...
                                            item.image ? <div className={item.imageClassName}>
                                                ...
                                                <div className="flex items-center justify-center w-10 h-10 bg-red-100 rounded-md shrink-0">
                                                    <ShoppingBag className="w-5 h-5 text-red-500 fill-red-200"></ShoppingBag>
                                                </div>
                                        }
                                        <div className="grow">
                                            ...
                                            <p className={`text-sm text-slate-500 dark:text-zink-300 ${index === notification.length - 3 ? "mb-3" : "mb-0"}`}>
                                                <Clock className="inline-block w-3.5 h-3.5 mr-1"></Clock> <span className="align-middle">{item.date}</span></p>
                    ...
                    <div className="flex items-center gap-2 p-4 border-t border-slate-200 dark:border-zink-500">
                        ...
                        <div className="shrink-0">
                            <button type="button" className="px-2 py-1.5 text-xs text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100">View All Notification
                                <MoveRight className="inline-block w-3.5 h-3.5 ml-1"></MoveRight>
                            </button>
                        </div>
                    </div>
                </Dropdown.Content >
